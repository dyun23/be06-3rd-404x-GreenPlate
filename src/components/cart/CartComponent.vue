            ...
            <div>
              <ul>
                <CartItemComponent v-for="item in items" :key="item.id" :item="item" :cartId="item.id"
                  @update-checked="updateItemChecked" @update-count="updateItemCount" @delete-item="deleteItem" />
              </ul>
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      deleteSelectedItems() {
        ...
        axios.post('http://localhost:8080/cart/delete/list', {
            cartIdList: selectedIds
          }, {
